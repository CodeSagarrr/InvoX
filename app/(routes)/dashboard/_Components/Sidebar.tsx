...
function Sidebar() {
  ...
  return (
    <>
      <section className={isMenuOpen ? `md:w-72 w-72 z-10 fixed md:h-screen h-screen rounded-md shadow-sm md:p-3 p-2 md:cursor-default cursor-pointer bg-white transition-all` : `md:w-72 w-10 z-10 fixed md:h-screen h-auto rounded-md shadow-sm md:p-3 p-2 md:cursor-default cursor-pointer bg-white transition-all mt-3 ml-2 md:mt-0 md:ml-0`}>

        <div onClick={handleMenu} className="w-full h-auto md:hidden block">
          {
            isMenuOpen ? (
              <X className="w-6 h-6 border rounded-lg p-0.5 float-end bg-red-500 text-amber-50" />
            ) : (
              <Menu className="w-5 h-5 ml-0.5" />
            )
          }
        </div>
